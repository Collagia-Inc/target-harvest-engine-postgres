...
class TestIntegration(unittest.TestCase):
    ...
    def remove_custom_column_from_rows(self, rows):
        """Removes metadata columns from a list of rows"""
        d_rows = []
        for r in rows:
            # Copy the original row to a new dict to keep the original dict
            # and remove metadata columns
            d_row = r.copy()
            d_row.pop(self.config["custom_column"], None)
    ...
    def assert_custom_column_exist(self, rows):
        """This is a helper assertion that checks if every row in a list has metadata columns"""
        for r in rows:
            self.assertTrue(self.config["custom_column"] in r)

    def assert_custom_column_not_exist(self, rows):
        """This is a helper assertion that checks metadata columns don't exist in any row"""
        for r in rows:
            self.assertFalse(self.config["custom_column"] in r)
    ...
    def assert_multiple_streams_are_into_postgres(self, should_metadata_columns_exist=False,
                                                  should_hard_deleted_rows=False, should_custom_column_exists=False):
        """
        ...
        if not should_custom_column_exists:
            self.assertEqual(
        ...
        if should_custom_column_exists:
            self.assert_custom_column_exist(table_one)
            self.assert_custom_column_exist(table_two)
            self.assert_custom_column_exist(table_three)
            self.assert_custom_column_exist(table_four)
        else:
            self.assert_custom_column_not_exist(table_one)
            self.assert_custom_column_not_exist(table_two)
            self.assert_custom_column_not_exist(table_three)
            self.assert_custom_column_not_exist(table_four)
    ...
    def test_loading_tables_with_custom_column(self):
        """Loading multiple tables from the same input tap with various columns types"""
        tap_lines = test_utils.get_test_tap_lines('messages-with-multiple-streams.json')

        # Turning on adding custom column
        self.config['custom_column'] = "connection_pk"
        self.config['custom_column_value'] = "101"
        target_postgres.persist_lines(self.config, tap_lines)

        # Check if data loaded correctly and custom columns exist
        self.assert_multiple_streams_are_into_postgres(should_custom_column_exists=True)
