...
def persist_lines(config, lines) -> None:
    ...
    for line in lines:
        ...
        if t == 'RECORD':
            ...
            if config.get('custom_column') and config.get('custom_column_value'):
                records_to_load[stream][primary_key_string][config["custom_column"]] = config["custom_column_value"]
            ...
            if config.get('custom_column') and config.get('custom_column_value'):
                o['schema']['properties'][config["custom_column"]] = {'type': ['null', 'integer']}
