...
def validate_config(config):
    ...
    config_custom_schema_mapping = config.get('custom_schema_mapping', None)
    if not config_default_target_schema and not config_schema_mapping and not config_custom_schema_mapping:
...
class DbSync:
    def __init__(self, connection_config, stream_schema_message=None):
        ...
        if stream_schema_message is not None:
            ...
            if self.connection_config.get('custom_schema_mapping'):
                stream_schema_name = self.connection_config.get('custom_schema_mapping', "").split(".")[0]
                stream_table_name = self.connection_config.get('custom_schema_mapping', "").split(".")[1]
    ...
    def table_name(self, stream_name, is_temporary=False, without_schema=False):
        ...
        if self.connection_config.get("custom_schema_mapping"):
            table_name = self.connection_config.get('custom_schema_mapping', "").split(".")[1]
    ...
    def load_csv(self, file, count, size_bytes):
        ...
        with self.open_connection() as connection:
            with connection.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                ...
                self.logger.info("temp table create query",self.create_table_query(table_name=temp_table, is_temporary=True))
                copy_sql = "COPY {} ({}) FROM STDIN WITH (FORMAT CSV, ESCAPE '\\')".format(
                    temp_table,
                    ', '.join(self.column_names())
                )
                self.logger.info("copy data to temp table", copy_sql)
                ...
                # output = self.query('SELECT * FROM he_app.{}'.format(temp_table))
                # self.logger.info("copy output temp table",output)
                
                self.logger.info("insert temp table  query",self.insert_from_temp_table(temp_table))
